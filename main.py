...
def count_down(count):
    ...
    if count > 0:
        ...
        marks =""
        work_sessions = math.floor(periods/2)
        for i in range(work_sessions):
            marks += "✔️"
        checking_label.config(text = marks)
        start_time()
...
start_button = Button(text = "Start", bg = "white", command = start_time)
start_button.grid(column = 0, row = 2)

reset_button = Button(text = "Reset", bg = "white", command = reset)
reset_button.grid(column = 2, row = 2)

checking_label = Label(bg = YELLOW, fg = GREEN, font = (FONT_NAME,20,"bold"))
